...
WINDOWSANE_API FREObject getWindowHeight(FREContext ctx,
										 void*      pFuncData,
										 uint32_t   argc,
										 FREObject  argv[])
{
...
WINDOWSANE_API FREObject getWindowPosX(FREContext ctx,
									   void*      pFuncData,
									   uint32_t   argc,
									   FREObject  argv[])
{
...
WINDOWSANE_API FREObject getWindowPosY(FREContext ctx,
									   void*      pFuncData,
									   uint32_t   argc,
									   FREObject  argv[])
{
...
WINDOWSANE_API FREObject getWindowWidth(FREContext ctx,
										void*      pFuncData,
										uint32_t   argc,
										FREObject  argv[])
{
...
WINDOWSANE_API FREObject isMaximized(FREContext ctx,
									 void*      pFuncData,
									 uint32_t   argc,
									 FREObject  argv[])
{
	...
	// create object
	FRENewObjectFromInt32((int32_t) (bRet ? 1 : 0), &pRet);
...
WINDOWSANE_API FREObject maximize(FREContext ctx,
								  void*      pFuncData,
								  uint32_t   argc,
								  FREObject  argv[])
{
...
WINDOWSANE_API FREObject moveWindow(FREContext ctx,
									void*      pFuncData,
									uint32_t   argc,
									FREObject  argv[])
{
	   ...
	   nHeight > 0 )
	{
		...
		// move window if found
		if(hWnd)
			MoveWindow(hWnd, nPosX, nPosY, nWidth, nHeight, TRUE);
	}
										  ...
										  FREObject  argv[])
{
	...
	if(g_bIsFullScreen)
	{
		...
		// fullscreen/restore macro
		#define FSR_STEP(src, dest, step) src + ((((dest) - (src)) * (step)) / FSR_STEPS)

		// restore window rect
		for(int c = 0; c <= FSR_STEPS; c++)
			MoveWindow(hWnd,
					   FSR_STEP(mi.rcMonitor.left,                        g_rWnd.left,                 c),
					   FSR_STEP(mi.rcMonitor.top,                         g_rWnd.top,                  c),
					   FSR_STEP(mi.rcMonitor.right  - mi.rcMonitor.left,  g_rWnd.right  - g_rWnd.left, c),
					   FSR_STEP(mi.rcMonitor.bottom - mi.rcMonitor.top,   g_rWnd.bottom - g_rWnd.top,  c),
					   TRUE);
	...
	else
	{
		...
		// fullscreen/restore macro
		#define FSR_STEP(dest, src, step) src + ((((dest) - (src)) * (step)) / FSR_STEPS)
